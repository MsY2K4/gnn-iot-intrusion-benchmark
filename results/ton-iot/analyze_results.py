...
Processes multi-seed GNN experiment results (TON_IoT dataset) across three
class-imbalance handling strategies:
    - unweighted_loss
    - weighted_loss
    - GraphSMOTE

and produces the tables needed for a results section / paper:
...
Input files (edit INPUT_FILES below if paths differ):
    ton_iot_unweighted_loss_all_architectures_multiseed.json
    ton_iot_weighted_loss_all_architectures_multiseed.json
    ton_iot_GraphSMOTE_all_architectures_multiseed.json
...
def flatten_overall(payloads):
    ...
    for payload in payloads:
        ...
        for arch_name, arch in architectures.items():
            ...
            n_seeds = arch.get("n_seeds", len(arch.get("seeds", [])))
            ...
            def get_metric(metric_key):
                ...
                vals = [
                    r.get(metric_key)
                    for r in arch.get("per_seed_results", [])
    ...
    for payload in payloads:
        ...
        for arch_name, arch in architectures.items():
            ...
            if per_class:
                ...
                per_seed = arch.get("per_seed_results", [])
...
def write_markdown_report(path, tables: dict):
    lines = ["# TON_IoT Multi-Seed Results Summary\n"]
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=None,
        help="Explicit list of JSON file paths (overrides --input-dir + defaults).",
    )
    parser.add_argument(
        "--output-dir",
    ...
    if args.files:
        ...
        paths = [os.path.join(args.input_dir, fn) for fn in DEFAULT_INPUT_FILES]
